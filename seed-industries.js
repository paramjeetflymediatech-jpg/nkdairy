...
const { Industry } = require('./src/models/Industry.ts');
...
async function seed() {
  console.log("--- Starting Database Seeding (Industries) ---");
  try {
    await sequelize.authenticate();
    
    for (const ind of industriesData) {
      // Find if slug already exists to prevent duplicates
      ...
      await Industry.create({
        ...
        image: ind.image,
        equipmentSolutions: JSON.stringify(ind.equipmentSolutions),
        faqs: JSON.stringify(ind.faqs),
